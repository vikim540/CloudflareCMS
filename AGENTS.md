...
> **強制約束文件**。所有代碼生成、修改、審查必須遵守。當前版本：**v1.9.74**（2026-08-25）
...
- 文章 `date` 字段作為發布時間，`status='0'` 為草稿（待發佈），`status='2'` 為草稿（不發佈），`status='1'` 為已發布
- Cron 每 15 分鐘掃描 24 小時內待發布文章（僅 `status='0'` 且有日期），投遞延遲消息到 Queue
- 已過期草稿（僅 `status='0'` 且有日期）直接在 Cron 中發布（兜底）
- `status='2'` 草稿（不發佈）永不被 Cron 發布，前端選擇時自動清空日期+禁用輸入框（v1.9.74）
- 後台「草稿」tab 用 `status=draft` 查詢 `status IN ('0','2')`，徽章區分：🟠待發佈 / ⚪草稿 / 🟡不發佈
