...
> **強制約束文件**。所有代碼生成、修改、審查必須遵守。當前版本：**v1.9.75**（2026-08-25）
...
- 文章 `date` 字段作為發布時間，`status='0'` 為草稿，`status='1'` 為已發布（API 不區分 status='2'）
- Cron 每 15 分鐘掃描 24 小時內待發布文章（僅 `status='0'` 且有日期），投遞延遲消息到 Queue
- 已過期草稿（僅 `status='0'` 且有日期）直接在 Cron 中發布（兜底）
- 草稿無日期時不被 Cron 發布（v1.9.70 修復：空日期草稿不會被誤發布）
- 前端編輯器下拉三選項：已發布 / 草稿（待發佈） / 草稿（不發佈），後兩者皆 status='0'，差別在有無日期
- 選擇「草稿（不發佈）」時自動清空日期 + 禁用日期輸入框（v1.9.75，前端用 '2' 作 UI 值，提交 API 時轉 '0'）
- 後台「草稿」tab 用 status=0 查全部草稿，徽章按日期區分：🟠待發佈 / ⚪草稿
