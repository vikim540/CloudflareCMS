...
> **強制約束文件**。所有代碼生成、修改、審查必須遵守。當前版本：**v1.9.72**（2026-08-25）
...
- 文章 `date` 字段作為發布時間，`status='0'` 為草稿（待發佈），`status='2'` 為草稿（不發佈，v1.9.72 新增）
- Cron 每 15 分鐘掃描 24 小時內待發布文章（僅 `status='0'`），投遞延遲消息到 Queue
- 已過期草稿（僅 `status='0'`）直接在 Cron 中發布（兜底）
- `status='2'` 草稿（不發佈）永不被 Cron 自動發布，供未定稿內容暫存
