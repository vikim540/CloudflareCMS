  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-secondary">
      <div className="w-full max-w-sm">
        <div className="bg-background rounded-lg shadow-level-3 border border-hairline p-8">
          <h1 className="text-2xl font-semibold tracking-tight text-center mb-6 text-foreground">CMS 管理後台</h1>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-medium mb-1.5 text-body">用戶名</label>
              <input
                type="text"
                value={username}
                onChange={(e) => setUsername(e.target.value)}
                className="w-full h-10 px-3 border border-hairline rounded-sm text-sm text-foreground bg-background focus:outline-none focus:border-ring focus:ring-2 focus:ring-ring/10 transition-colors"
                placeholder="admin"
                required
              />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1.5 text-body">密碼</label>
              <input
                type="password"
                value={password}
            ...
            <button
              ...
              className="w-full h-10 bg-primary text-primary-foreground rounded-sm text-sm font-medium hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed transition-opacity"
            >
              {loading ? '登錄中...' : '登錄'}
            </button>
          </form>
