...
🔧 設計原則
• API 只用 status='0'（草稿），不區分 status='2'
• 前端下拉三選項：已發布 / 草稿（待發佈）/ 草稿（不發佈）
  - 「草稿（不發佈）」= status='0' + 空日期（前端用 '2' 作 UI 值，提交時轉 '0'）
  - 「草稿（待發佈）」= status='0' + 有日期（Cron 到時自動發布）
• 選擇「不發佈」時自動清空日期 + 禁用日期輸入框，從根本防止誤發布

🔧 後端（content.ts）
• 移除所有 status='2' 條件，status=all 恢復 IN('0','1')
• handleAdminListContents：status='0' 查全部草稿（不再加日期條件）
• 公開 API status=0 預覽所有草稿

🔧 前端
• ContentEdit.tsx：'2' 純前端 UI 值，提交 API 時轉 '0'，originalDataRef 保留前端值
• Contents.tsx：單一草稿 tab（status=0），徽章按日期區分：🟠待發布/⚪草稿`,
  },
  {
    version: 'v1.9.74',
    ...
    icon: '📝',
    latest: false,
    changes: `📝 草稿狀態優化（已被 v1.9.75 取代：API 不再使用 status='2'）`,
  },
  {
    version: 'v1.9.73',
...
• 草稿防誤發布已由 v1.9.70 Cron 邏輯修復（空日期不發布），無需額外狀態

🔧 保留的改進
• 後台 tab 互斥設計：草稿(status=0) 排除未來日期，與待發布(scheduled) 不重疊
• 公開 API status=0 仍可預覽所有草稿（含待發布+純草稿），前端預覽連接正常

🔧 回退內容
• content.ts：移除 status='2' 相關條件，status=all 恢復 IN('0','1')
• ContentEdit.tsx：移除「草稿（不發佈）」選項，恢復「草稿」標籤
• Contents.tsx：移除「草稿（不發佈）」tab + 黃色徽章，恢復 4 個 tab`,
  },
  {
    version: 'v1.9.72',
    ...
    icon: '📝',
    latest: false,
    changes: `📝 草稿狀態細分：草稿（待發佈）vs 草稿（不發佈）（已被 v1.9.73 回退）
...
const API_ENDPOINTS: ApiEndpoint[] = [
  ...
  // 管理接口 (300次/分/用戶)
  { method: 'GET', path: '/api/v1/admin/contents', desc: '後台內容列表 (?scode=&mcode=&status=&page=, status: 1=已發布/0=草稿/all=全部/scheduled=待發布 v1.9.75+)', auth: true },
